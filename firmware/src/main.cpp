...
void loop() {
  ...
  // Redraw on a new payload, on a link state change, or on a slow heartbeat.
  // display::render() diffs against what is already on the glass, so a
  // heartbeat with nothing new costs a couple of comparisons.
